...
rp::opengl::ViewerData::ViewerData()
...
void rp::opengl::ViewerData::set_face_based(bool newvalue)
{
...
void rp::opengl::ViewerData::set_mesh(
    const Eigen::MatrixXd& _V, const Eigen::MatrixXi& _F)
{
  ...
  if (V.rows() == 0 && F.rows() == 0)
  {
    ...
    compute_normals();
    uniform_colors(
      Eigen::Vector3d(igl::GOLD_AMBIENT[0], igl::GOLD_AMBIENT[1], igl::GOLD_AMBIENT[2]),
      Eigen::Vector3d(igl::GOLD_DIFFUSE[0], igl::GOLD_DIFFUSE[1], igl::GOLD_DIFFUSE[2]),
      Eigen::Vector3d(igl::GOLD_SPECULAR[0], igl::GOLD_SPECULAR[1], igl::GOLD_SPECULAR[2]));
...
void rp::opengl::ViewerData::set_vertices(const Eigen::MatrixXd& _V)
{
  V = _V;
...
void rp::opengl::ViewerData::set_normals(const Eigen::MatrixXd& N)
{
  using namespace std;
  if (N.rows() == V.rows())
  {
...
void rp::opengl::ViewerData::set_visible(bool value, unsigned int core_id /*= 1*/)
{
  if (value)
    is_visible |= core_id;
  else
...
void rp::opengl::ViewerData::copy_options(const ViewerCore &from, const ViewerCore &to)
{
...
void rp::opengl::ViewerData::set_colors(const Eigen::MatrixXd &C)
{
  using namespace std;
  ...
  enable_angle_color = false;
  dirty |= MeshGL::DIRTY_DIFFUSE | MeshGL::DIRTY_SPECULAR | MeshGL::DIRTY_AMBIENT;

}

void rp::opengl::ViewerData::set_angles(const Eigen::VectorXd &a, const Eigen::VectorXd &da,
                                        const Eigen::MatrixXi &EV, const Eigen::MatrixXi &FE) {
  if (a.rows() != V.rows()) {
    std::cerr << "invalid input of a" << std::endl;
    return set_data(a);
  } else if (da.rows() != EV.rows()) {
    std::cerr << "invalid input of da" << std::endl;
    return set_data(a);
  } else if (EV.cols() != 2 || FE.cols() != 3 || FE.rows() != F.rows() || FE.maxCoeff() >= EV.rows()) {
    std::cerr << "invalid input of FE" << std::endl;
    return set_data(a);
  }

  FV_angles = Eigen::VectorXd(F.rows() * 3);
  F_indices = Eigen::VectorXd(F.rows());

  for (int i = 0; i < F.rows(); i++) {
    double angle = a(F(i, 0));
    double turningNum = 0;

    FV_angles(3 * i) = angle;
    for (int j = 0; j < 3; j++) {
      int id = 3 * i + j;
      int v0 = F(i, (j + 2) % 3), v1 = F(i, j);
      double diff = 0;
      double sign = 0;
      ...
      for (int k = 0; k < 3; k++) {
        ...
        if (EV(eid, 0) == v0 && EV(eid, 1) == v1) {
          sign = 1;
          diff = da(eid);
          break;
        } else if (EV(eid, 0) == v1 && EV(eid, 1) == v0) {
          sign = -1;
          diff = da(eid);
          break;
        }
      ...
      turningNum += sign * diff;
    }

    F_indices(i) = turningNum / (2 * igl::PI);

    for (int j = 1; j < 3; j++) {
      int id = 3 * i + j;
      double sub = j * turningNum / 3;
      FV_angles(id) -= sub;
    }
  }

  enable_angle_color = true;

  dirty |= MeshGL::DIRTY_CUSTOM_LABELS;
}

void rp::opengl::ViewerData::set_uv(const Eigen::MatrixXd& UV)
{
  using namespace std;
  if (UV.rows() == V.rows())
  {
...
void rp::opengl::ViewerData::set_uv(const Eigen::MatrixXd& UV_V, const Eigen::MatrixXi& UV_F)
{
  set_face_based(true);
...
void rp::opengl::ViewerData::set_texture(
...
void rp::opengl::ViewerData::set_data(
...
void rp::opengl::ViewerData::set_data(const Eigen::VectorXd & D, igl::ColorMapType cmap, int num_steps)
{
...
void rp::opengl::ViewerData::set_colormap(const Eigen::MatrixXd & CM)
{
...
void rp::opengl::ViewerData::set_points(
...
void rp::opengl::ViewerData::add_points(const Eigen::MatrixXd& P,  const Eigen::MatrixXd& C)
{
...
void rp::opengl::ViewerData::clear_points()
{
  points.resize(0, 6);
}

void rp::opengl::ViewerData::set_edges(
  const Eigen::MatrixXd& P,
...
void rp::opengl::ViewerData::set_edges_from_vector_field(
  const Eigen::MatrixXd& P, 
...
void rp::opengl::ViewerData::add_edges(const Eigen::MatrixXd& P1, const Eigen::MatrixXd& P2, const Eigen::MatrixXd& C)
{
...
void rp::opengl::ViewerData::clear_edges()
{
  lines.resize(0, 9);
}

void rp::opengl::ViewerData::add_label(const Eigen::VectorXd& P,  const std::string& str)
{
...
void rp::opengl::ViewerData::set_labels(const Eigen::MatrixXd& P, const std::vector<std::string>& str)
{
...
void rp::opengl::ViewerData::clear_labels()
{
  labels_positions.resize(0,3);
  labels_strings.clear();
}

void rp::opengl::ViewerData::clear()
{
  ...
  FV_angles               = Eigen::VectorXd (0);
  F_indices               = Eigen::VectorXd (0);
  enable_angle_color      = false;
...
void rp::opengl::ViewerData::compute_normals()
{
  if(V.cols() == 2)
  {
...
void rp::opengl::ViewerData::uniform_colors(
...
void rp::opengl::ViewerData::normal_matcap()
{
  const int size = 512;
...
void rp::opengl::ViewerData::grid_texture()
{
  unsigned size = 128;
...
void rp::opengl::ViewerData::update_labels(
  rp::opengl::MeshGL& meshgl,
  rp::opengl::MeshGL::TextGL& GL_labels,
...
void rp::opengl::ViewerData::updateGL(
  const rp::opengl::ViewerData& data,
  ...
  )
{
  ...
  if (!data.face_based)
  {
    ...
    if (meshgl.dirty & MeshGL::DIRTY_CUSTOM_LABELS) {
      if (data.FV_angles.rows() == data.F.rows() * 3) {
        meshgl.V_angle_vbo.resize(data.F.rows() * 3);
        for (int i = 0; i < data.FV_angles.rows(); i++) {
          meshgl.V_angle_vbo(i) = data.FV_angles(i);
        }
      }

      if (data.F_indices.rows() == F.rows()) {
        meshgl.V_index_vbo.resize(data.F.rows() * 3);
        for (int i = 0; i < data.F_indices.rows(); i++) {
          for (int j = 0; j < 3; j++) {
            meshgl.V_index_vbo(3 * i + j) = data.F_indices(i);
          }
        }
      }

      meshgl.V_indexInFace_vbo = Eigen::VectorXf(data.F.rows() * 3);
      for (int i = 0; i < data.F.rows(); i++) {
        for (int j = 0; j < 3; j++) {
          meshgl.V_indexInFace_vbo(3 * i + j) = (float)j;
        }
      }
    }

    meshgl.enable_angle_color = data.enable_angle_color;
    // }
  } else
  {
    if (meshgl.dirty & MeshGL::DIRTY_POSITION)
    ...
    if (meshgl.dirty & MeshGL::DIRTY_CUSTOM_LABELS) {
      if (data.FV_angles.rows() == data.F.rows() * 3) {
        meshgl.V_angle_vbo = Eigen::VectorXf(data.F.rows() * 3);
        for (int i = 0; i < data.FV_angles.rows(); i++) {
          meshgl.V_angle_vbo(i) = data.FV_angles(i);
        }
      }
      ...
      if (data.F_indices.rows() == F.rows()) {
        meshgl.V_index_vbo = Eigen::VectorXf(data.F.rows() * 3);
        for (int i = 0; i < data.F_indices.rows(); i++) {
          for (int j = 0; j < 3; j++) {
            meshgl.V_index_vbo(3 * i + j) = data.F_indices(i);
          }
