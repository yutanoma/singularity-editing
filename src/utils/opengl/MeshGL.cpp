...
void rp::opengl::MeshGL::init_buffers()
{
  ...
  glGenBuffers(1, &vbo_V_angle);
  glGenBuffers(1, &vbo_V_index);
  glGenBuffers(1, &vbo_V_indexInFace);
...
void rp::opengl::MeshGL::free_buffers()
{
  if (is_initialized)
  {
    ...
    glDeleteBuffers(1, &vbo_V_angle);
    glDeleteBuffers(1, &vbo_V_index);
    glDeleteBuffers(1, &vbo_V_indexInFace);
...
void rp::opengl::MeshGL::TextGL::init_buffers()
{
...
void rp::opengl::MeshGL::TextGL::free_buffers()
{
...
void rp::opengl::MeshGL::bind_mesh()
{
  ...
  bind_vertex_attrib_array(shader_mesh,"angle", vbo_V_angle, V_angle_vbo, dirty & MeshGL::DIRTY_CUSTOM_LABELS);
  bind_vertex_attrib_array(shader_mesh,"index", vbo_V_index, V_index_vbo, dirty & MeshGL::DIRTY_CUSTOM_LABELS);
  bind_vertex_attrib_array(shader_mesh,"indexInFace", vbo_V_indexInFace, V_indexInFace_vbo, dirty & MeshGL::DIRTY_CUSTOM_LABELS);
  ...
  glUniform1i(glGetUniformLocation(shader_mesh, "angle_color"), enable_angle_color);
  dirty &= ~MeshGL::DIRTY_MESH;
}

void rp::opengl::MeshGL::bind_overlay_lines()
{
  using namespace igl::opengl;
...
void rp::opengl::MeshGL::bind_overlay_points()
{
  using namespace igl::opengl;
...
void rp::opengl::MeshGL::init_text_rendering()
{
  using namespace igl::opengl;
...
void rp::opengl::MeshGL::bind_labels(const TextGL& labels)
{
  using namespace igl::opengl;
...
void rp::opengl::MeshGL::draw_mesh(bool solid)
{
...
void rp::opengl::MeshGL::init()
{
  ...
  void main()
  {
    ...
    anglei = angle;
    indexi = index;
    float zero = (-0.1 < indexInFace && indexInFace < 0.1) ? 1.0 : 0.0;
    float one = (0.9 < indexInFace && indexInFace < 1.1) ? 1.0 : 0.0;
    float two = (1.9 < indexInFace && indexInFace < 2.1) ? 1.0 : 0.0;
    barycentric_coord = vec3 (zero, one, two);
  }
)";
  ...
  void main()
  {
    ...
    if (angle_color) {
      float larg = 0;
      float t_i = barycentric_coord.x;
      float t_j = barycentric_coord.y;
      float t_k = barycentric_coord.z;
      if (t_k <= t_i && t_k <= t_j) {
        larg = PI * indexi * (1 + ((t_j - t_i) / (1 - 3 * t_k))) / 3;
      }else if (t_i <= t_j && t_i <= t_k) {
        larg = PI * indexi * (3 + ((t_k - t_j) / (1 - 3 * t_i))) / 3;
      }else if (t_j <= t_k && t_j <= t_i) {
        larg = PI * indexi * (5 + ((t_i - t_k) / (1 - 3 * t_j))) / 3;
      }
      float real_angle = larg + anglei;
      float deg = mod(degrees(real_angle), 360.0) / 360.0;
      
      // orange
      // Kdii = deg > 0.5 ? vec4(.91, .39, .29, 1.0) : vec4(.90, .71, .67, 1.0);
      // blue
      // Kdii = deg > 0.5 ? vec4(.31, .39, .61, 1.0) : vec4(.09, .15, .3, 1.0);
      // skyblue
      Kdii = deg > 0.5 ? vec4(.23, .47, .62, 1.0) : vec4(.49, .65, .73, 1.0);
      // grey
      // Kdii = deg > 0.5 ? vec4(.64, .64, .64, 1.0) : vec4(.32, .32, .32, 1.0);
      // Kdii = vec4(((clamp(abs(fract(deg+vec3(0,2,1)/3.)*6.-3.)-1.,0.,1.)-1.)*1.+1.)*1., 1.0);
      Kaii = vec4(Kdii.xyz / 10, 1.0);
      Ksii = vec4(Kdii.xyz / 10, 1.0);
    }
    ...
    {
      vec3 Ia = La * vec3(Kaii);    // ambient intensity

      vec3 vector_to_light_eye = light_position_eye - position_eye;
      vec3 direction_to_light_eye = normalize (vector_to_light_eye);
      float dot_prod = dot (direction_to_light_eye, normalize(normal_eye));
      float clamped_dot_prod = abs(max (dot_prod, -double_sided));
      vec3 Id = Ld * vec3(Kdii) * clamped_dot_prod;    // Diffuse intensity
      ...
      vec3 Is = Ls * vec3(Ksii) * specular_factor;    // specular intensity
      vec4 color = vec4(lighting_factor * (Is + Id) + Ia + (1.0-lighting_factor) * vec3(Kdii),(Kaii.a+Ksii.a+Kdii.a)/3);
