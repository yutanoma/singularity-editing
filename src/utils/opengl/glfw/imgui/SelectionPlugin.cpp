...
void SelectionPlugin::init(rp::opengl::glfw::Viewer *_viewer)
{
...
bool SelectionPlugin::pre_draw()
{
  if(mode == OFF){ return false; }
  ImGuiMenu::pre_draw();
  return false;
}
bool SelectionPlugin::post_draw()
{
  if(mode == OFF){ return false; }
...
bool SelectionPlugin::mouse_down(int button, int modifier)
{
...
bool SelectionPlugin::mouse_up(int button, int modifier)
{
  is_down = false;
...
bool SelectionPlugin::key_pressed(unsigned int key, int modifiers)
{
  Mode old = mode;
...
void SelectionPlugin::circle(const Eigen::Matrix<float,2,2> & M,  std::vector<Eigen::RowVector2f> & L)
{
  L.clear();
  L.reserve(64);
...
void SelectionPlugin::rect(const Eigen::Matrix<float,2,2> & M,  std::vector<Eigen::RowVector2f> & L)
{
  L.resize(4);
...
Eigen::RowVector2f SelectionPlugin::xy(const Viewer * vr)
{
