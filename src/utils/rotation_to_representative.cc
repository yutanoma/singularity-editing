#include "rotation_to_representative.h"
...
void rotation_to_representative(
  ...
  for (int i = 0; i < EF.rows(); i++) {
    for (int j = 0; j < 2; j++) {
      ...
      edgeRep(i, j) = pow(Complex(edgeVector.dot(B1.row(EF(i, j))),
                                  edgeVector.dot(B2.row(EF(i, j)))),
                          (double)N);
    }
  }
  ...
  for (int i = 0; i < EF.rows(); i++) {
    ...
    aP1FullTriplets.push_back(Triplet<Complex>(
        i, EF(i, 0),
        conj(edgeRep(i, 0)) * exp(Complex(0, (double)N * rotationAngles(i)))));
    aP1FullTriplets.push_back(
        Triplet<Complex>(i, EF(i, 1), -conj(edgeRep(i, 1))));
    if (EF(i, 0) != 0)
      aP1Triplets.push_back(
          Triplet<Complex>(i, EF(i, 0) - 1,
                           conj(edgeRep(i, 0)) *
                               exp(Complex(0, (double)N * rotationAngles(i)))));
    if (EF(i, 1) != 0)
      aP1Triplets.push_back(
          Triplet<Complex>(i, EF(i, 1) - 1, -conj(edgeRep(i, 1))));
  }
  aP1Full.setFromTriplets(aP1FullTriplets.begin(), aP1FullTriplets.end());
  aP1.setFromTriplets(aP1Triplets.begin(), aP1Triplets.end());
  VectorXcd torhs = VectorXcd::Zero(F.rows());
  torhs(0) = globalRot;  // global rotation
  ...
  for (int i = 0; i < F.rows(); i++) {
    RowVector3d currVector =
        B1.row(i) * complexField(i).real() + B2.row(i) * complexField(i).imag();
    // std::cout<<"currVector: "<<currVector<<std::endl;
    representative.row(i) = currVector.normalized();
  }

  std::cout << "rotation_to_representative linfError: " << (aP1 * complexPowerField.tail(F.rows() - 1) - rhs).norm() << std::endl;
}
...
void rotation_to_representative(const Eigen::MatrixXd& V,
  ...
  rp::rotation_to_representative(V, F, EV, EF, B1, B2, rotationAngles, N,
                                 globalRotation, representative);
}
