...
class Mesh {
 ...
 private:
  ...
  bool verify();  // verifies connectivity of the mesh and prints some debug
                  // info
  ...
  SimlpeMemoryAllocator<void_pointer>
      m_pointer_allocator;  // fast memory allocating for Face/Vertex/Edge
                            // cross-references
};
...
void Mesh::initialize_mesh_data(unsigned num_vertices, Points& p,
       ...
       ++i)  // copy adjacent vertices to polygons/faces
  {
    ...
    for (unsigned j = 0; j < 3; ++j) {
      ...
      // std::cout << vertex_index << ", " << num_vertices << std::endl;
...
inline void Mesh::build_adjacencies() {
  ...
  for (unsigned i = 1; i < half_edges.size(); ++i) {
    if (half_edges[i] != half_edges[i - 1]) {
      ...
      if (i <
          half_edges.size() -
              1)  // sanity check: there should be at most two equal half-edges
  ...
  for (unsigned i = 0; i < m_vertices.size(); ++i) {
    ...
    v.saddle_or_boundary() =
        (total_vertex_angle[v.id()] > 2.0 * igl::PI - 1e-5);
  }
...
class Interval  // interval of the edge
{
 public:
  ...
  void find_closest_point(
      double const x, double const y, double& offset,
      double& distance);  // find the point on the interval that is closest to
                          // the point (alpha, s)
...
class GeodesicAlgorithmBase {
 public:
  ...
  virtual unsigned best_source(
      SurfacePoint& point,  // after propagation step is done, quickly find what
                            // source this point belongs to and what is the
...
class GeodesicAlgorithmExact : public GeodesicAlgorithmBase {
 public:
  ...
  unsigned best_source(
      SurfacePoint& point,  // quickly find what source this point belongs to
 ...
 private:
  ...
  unsigned intersect_intervals(
      ...
      IntervalWithStop* one);  // intersecting two intervals with up to three
...
inline void GeodesicAlgorithmExact::propagate(
    std::vector<SurfacePoint>& sources,
    double
        max_propagation_distance,  // propagation algorithm stops after reaching
  ...
  while (!m_queue.empty()) {
    m_queue_max_size =
        std::max(static_cast<unsigned int>(m_queue.size()), m_queue_max_size);
...
inline void GeodesicAlgorithmExact::update_list_and_queue(
  ...
  for (unsigned i = 0; i < num_candidates; ++i)  // for all new intervals
  {
               ...
               q->stop() - local_epsilon)  // go through all old intervals
    {
      ...
      if (N == 1) {
        if (map[0] ==
            OLD)  // if "p" is always better, we do not need to update anything)
        {
    ...
    IntervalWithStop* candidates)  // if it is the last interval on the edge
{
  ...
  double L2 = compute_positive_intersection(end, pseudo_x, pseudo_y, sin_alpha,
                                            cos_alpha);
  ...
  if (best_interval)  // if we did not hit the face source immediately
  {
            ...
            // source (if we are inside the FACE, we obviously hit the source)
    {
      ...
      if (position < local_epsilon) {
        path.emplace_back(SurfacePoint(e->v0()));
      } else if (position > e->length() - local_epsilon) {
        path.emplace_back(SurfacePoint(e->v1()));
      } else {
        double normalized_position = position / e->length();
        path.emplace_back(SurfacePoint(e, normalized_position));
      }
    }
  }
...
template <typename DerivedV, typename DerivedF, typename DerivedVS,
          ...
          typename DerivedD>
inline void exact_geodesic::exact_geodesic(
  ...
  assert(VS.cols() <= 1 && FS.cols() <= 1 && VT.cols() <= 1 && FT.cols() <= 1 &&
         "Only support one dimensional inputs");
  std::vector<Eigen::MatrixX3d> points(V.rows() * V.cols());
  std::vector<Eigen::MatrixX3i> faces(F.rows() * F.cols());
  ...
  exact_geodesic::geodesic::Mesh mesh;
  mesh.initialize_mesh_data(points, faces);
  exact_geodesic::geodesic::GeodesicAlgorithmExact exact_algorithm(&mesh);

  std::vector<exact_geodesic::geodesic::SurfacePoint> source(VS.rows() +
                                                             FS.rows());
  std::vector<exact_geodesic::geodesic::SurfacePoint> target(VT.rows() +
                                                             FT.rows());
  for (int i = 0; i < VS.rows(); i++) {
    source[i] =
        (exact_geodesic::geodesic::SurfacePoint(&mesh.vertices()[VS(i, 0)]));
  }
  for (int i = 0; i < FS.rows(); i++) {
    source[i] =
        (exact_geodesic::geodesic::SurfacePoint(&mesh.faces()[FS(i, 0)]));
  }

  for (int i = 0; i < VT.rows(); i++) {
    target[i] =
        (exact_geodesic::geodesic::SurfacePoint(&mesh.vertices()[VT(i, 0)]));
  }
  for (int i = 0; i < FT.rows(); i++) {
    target[i] =
        (exact_geodesic::geodesic::SurfacePoint(&mesh.faces()[FT(i, 0)]));
  }

  // sourceは一回ずつpropagateし直す
  exact_algorithm.propagate(source);
  std::vector<exact_geodesic::geodesic::SurfacePoint> path;
  D.resize(target.size(), 1);
  for (int i = 0; i < target.size(); i++) {
    exact_algorithm.trace_back(target[i], path);
    D(i) = exact_geodesic::geodesic::length(path);
  }
}

void exact_geodesic::get_crack_path(Eigen::MatrixX3d& V, Eigen::MatrixX3i& F,
                                    const int p1, const int p2,
                                    std::vector<int>& result_path) {
  std::vector<double> points(V.rows() * V.cols());
  std::vector<int> faces(F.rows() * F.cols());
  ...
  // 最終的なpathを格納する場所
  std::vector<std::vector<exact_geodesic::geodesic::SurfacePoint>> paths;

  std::vector<exact_geodesic::geodesic::SurfacePoint> source(1);
  std::vector<exact_geodesic::geodesic::SurfacePoint> target(1);

  source[0] = (exact_geodesic::geodesic::SurfacePoint(&mesh.vertices()[p1]));
  target[0] = (exact_geodesic::geodesic::SurfacePoint(&mesh.vertices()[p2]));
  ...
  for (int i = 0; i < path.size(); i++) {
    ...
    if (point.base_element()->type() == exact_geodesic::geodesic::VERTEX) {
      next_vid = point.base_element()->id();
    } else if (point.base_element()->type() == exact_geodesic::geodesic::EDGE) {
      // 常に右側のポイントを挿入する
      auto& adjacent_vertices = point.base_element()->adjacent_vertices();

      if (adjacent_vertices.size() != 2) {
        assert(false && "size of adjacent vertices is not 2");
      }

      // 一個前のポイントと現在のポイントが共有している三角形を算出
      int fid = -1;
      if (i > 0) {
        auto prev_point = path[i - 1];

        for (int j = 0; j < point.base_element()->adjacent_faces().size();
             j++) {
          for (int k = 0;
               k < prev_point.base_element()->adjacent_faces().size(); k++) {
            if (point.base_element()->adjacent_faces()[j]->id() ==
                prev_point.base_element()->adjacent_faces()[k]->id()) {
              fid = point.base_element()->adjacent_faces()[j]->id();
              break;
            }
          }
          if (fid != -1) {
        ...
        assert(false && "start of path should be a vertex");
      }

      if (fid == -1) {
        assert(false && "fid is still -1");
      }

      for (int j = 0; j < 3; j++) {
        auto vertex = F(fid, j);

        if (vertex == adjacent_vertices[0]->id()) {
          if (F(fid, (j + 2) % 3) == adjacent_vertices[1]->id()) {
            // adjacent_vertices[1]->id()が右側にある
            next_vid = adjacent_vertices[1]->id();
          } else if (F(fid, (j + 1) % 3) == adjacent_vertices[1]->id()) {
            // 0が右側にある
            next_vid = adjacent_vertices[0]->id();
          } else {
            assert(false && "no vertex in common face");
          }
        }
      }
    }
    ...
    if (next_vid == -1) {
      assert(false && "next vid is still 0");
    }

    if (result_path.size() > 0 && next_vid != -1) {
      if (result_path[result_path.size() - 1] != next_vid) {
        result_path.emplace_back(next_vid);
      }
    } else {
      result_path.emplace_back(next_vid);
    }
  ...
  // 最後に、もし重複があればその間にあるものをまるまる削除する
  std::vector<int> new_result_path = {};
  std::vector<int> is_in_path_idx(max_vid + 1);

  for (int i = 0; i < is_in_path_idx.size(); i++) {
    is_in_path_idx[i] = -1;
  }

  for (int i = 0; i < result_path.size(); i++) {
    if (is_in_path_idx[result_path[i]] != -1) {
      // 重複があったということなので、遡って消す
      new_result_path.resize(0);
      std::cout << result_path[i] << ", " << i << std::endl;
      for (int j = 0; j <= is_in_path_idx[result_path[i]]; j++) {
        new_result_path.emplace_back(result_path[j]);
      }
    } else {
      new_result_path.emplace_back(result_path[i]);
      is_in_path_idx[result_path[i]] = new_result_path.size() - 1;
    }
  }
...
void exact_geodesic::split_path(const Eigen::MatrixX3d& V, const Eigen::MatrixX3i& F,
                                // 切り開く点のindices
                                const std::vector<Eigen::VectorXi>& positions,
                                // 切り開いた箇所の座標
                                std::vector<Eigen::MatrixX3d>& result_points) {
  std::vector<Eigen::VectorXi> r;
  Eigen::MatrixX3d _V = V;
  Eigen::MatrixX3i _F = F;
  std::vector<std::vector<std::pair<std::array<int, 2>, double>>> edgePaths = {};

  exact_geodesic::split_path(_V, _F, positions, false, result_points, r, edgePaths);
};

// 点の位置からパスを作る。upadte_geometryをtrueにすると当該のパスで三角形を分割する。
void exact_geodesic::split_path(Eigen::MatrixX3d& V, Eigen::MatrixX3i& F,
                                // 切り開く点のindices
                                const std::vector<Eigen::VectorXi>& positions,
  ...
  exact_geodesic::split_path(V, F, positions, update_geometry, result_points,
                             result_indices, edgePaths);
}

void exact_geodesic::split_path(
  ...
  // パスのvertexのindex
  // update_geometryがtrueだったら空配列
  // 最終的にresult_indicesに変換する
  std::vector<std::vector<int>> path_indices = {};

  for (int j = 0; j < positions.size(); j++) {
    // std::cout << "exact_geodesic: checkpoint 0.00" << std::endl;

    // std::cout << F.rows() << ", " << V.rows() << std::endl;

    // 前回のイテレーションでメッシュが変わった場合はもう一度やり直す
    if (j > 0 && update_geometry) {
      points.resize(V.rows() * V.cols());
      faces.resize(F.rows() * F.cols());
      ...
      mesh.initialize_mesh_data(points, faces);
      exact_algorithm = exact_geodesic::geodesic::GeodesicAlgorithmExact(&mesh);
    }

    // std::cout << "exact_geodesic: checkpoint 0.0" << std::endl;
    ...
    // まず、最短距離のパスを作る
    {
      ...
      for (int i = 0; i < position.rows() - 1; i++) {
        int vid = position(i);
        int next_vid = position(i + 1);

        // std::cout << "exact_geodesic: checkpoint 0.1" << std::endl;

        std::vector<exact_geodesic::geodesic::SurfacePoint> source(1);
        source[0] =
            exact_geodesic::geodesic::SurfacePoint(&mesh.vertices()[vid]);

        auto next_vertex =
            exact_geodesic::geodesic::SurfacePoint(&mesh.vertices()[next_vid]);

        // sourceは一回ずつpropagateし直す
        exact_algorithm.propagate(source);
        exact_algorithm.trace_back(next_vertex, path);

        // pathの中にはnext, ...,
        // currentの順に入っているので、逆向きからpathsに入れていく
        int path_size = path.size();
        for (int k = 0; k < path.size(); k++) {
          paths[j].emplace_back(path[path_size - 1 - k]);
        }
      }
    }

    // std::cout << "exact_geodesic: checkpoint 0.2" << std::endl;

    // 次に、最短距離のパスの座標を結果に格納する
    {
      auto path = paths[j];
      Eigen::MatrixX3d matrix(path.size(), 3);
      ...
      for (int i = 0; i < path.size(); i++) {
        ...
        if (path[i].base_element()->type() == rp::exact_geodesic::geodesic::PointType::EDGE) {
          int v1 = path[i].base_element()->adjacent_vertices()[0]->id();
          int v2 = path[i].base_element()->adjacent_vertices()[1]->id();
          std::array<int, 2> arr = {v1, v2};

          assert(v1 < v2 || v1 == -1 || v2 == -1);

          double ratio = path[i].distance(path[i].base_element()->adjacent_vertices()[0]) / path[i].base_element()->adjacent_vertices()[0]->distance(path[i].base_element()->adjacent_vertices()[1]);
          // std::cout << ratio << std::endl;

          bool isnew = false;
          if (i < 1) {
            isnew = true;
          } else if (path[i - 1].type() == rp::exact_geodesic::geodesic::PointType::VERTEX) {
            isnew = true;
          }

          if (isnew) {
            std::vector<std::pair<std::array<int, 2>, double>> a = {std::pair<std::array<int, 2>, double>(arr, ratio)};
            edgePaths.emplace_back(a);
          } else {
            edgePaths[edgePaths.size() - 1].emplace_back(std::pair<std::array<int, 2>, double>(arr, ratio));
          }
        }
      }

      result_points.emplace_back(matrix);
    }

    // もしupdate_geometryがtrueなら、今回作ったパスで形状を分解する
    if (update_geometry) {
      // std::cout << "exact_geodesic: checkpoint 1" << std::endl;

      {
        path_indices.push_back({});

        auto path = paths[j];
        std::vector<exact_geodesic::geodesic::SurfacePoint> new_vertex_points =
            {};

        int new_vertex_index_start = V.rows();
        int new_vertices_count = 0;

        // std::cout << "exact_geodesic: checkpoint 2" << std::endl;

        // 一度目の走査では点を追加する
        for (int i = 0; i < path.size(); i++) {
          auto p = path[i];

          if (p.type() == exact_geodesic::geodesic::VERTEX) {
            path_indices[j].emplace_back(p.base_element()->id());
          } else if (p.type() == exact_geodesic::geodesic::EDGE) {
            // 新しいvidを付与する
            path_indices[j].emplace_back(new_vertex_index_start +
                                         new_vertices_count);

            new_vertex_points.emplace_back(p);
            new_vertices_count++;
          }
        }

        // std::cout << "exact_geodesic: checkpoint 3" << std::endl;

        V.conservativeResize(V.rows() + new_vertices_count, 3);

        // std::cout << "exact_geodesic: checkpoint 4" << std::endl;

        for (int i = 0; i < new_vertex_points.size(); i++) {
          auto p = new_vertex_points[i];
          V(new_vertex_index_start + i, 0) = p.x();
          V(new_vertex_index_start + i, 1) = p.y();
          V(new_vertex_index_start + i, 2) = p.z();
        }

        // std::cout << "exact_geodesic: checkpoint 5" << std::endl;

        std::map<int, std::vector<int>> fid_vid_pair;

        rp::exact_geodesic::geodesic::SurfacePoint next_p = path[0];
        int next_p_index = path_indices[j][0];

        // 2度目の走査では、追加された点のリストで辺を開く
        for (int i = 0; i < path.size() - 1; i++) {
          // 今と次のpに挟まれた三角形を分割する
          auto p = next_p;
          auto p_index = next_p_index;

          // next_pは常に最新であり続ける
          next_p = path[i + 1];
          next_p_index = path_indices[j][i + 1];

          // std::cout << "exact_geodesic: checkpoint 6." << i << std::endl;

          if (p.type() == exact_geodesic::geodesic::VERTEX &&
              next_p.type() == exact_geodesic::geodesic::VERTEX) {
            // 両方とも元からある点
            continue;
          } else if (next_p.type() == exact_geodesic::geodesic::FACE) {
            next_p = p;
            next_p_index = p_index;
            continue;
          } else {
            // pとnext_pのどちらか、またはその両方がedgeにある
            auto p_faces = p.base_element()->adjacent_faces();
            auto np_faces = next_p.base_element()->adjacent_faces();
            exact_geodesic::geodesic::face_pointer current_face;

            for (auto f : p_faces) {
              for (auto nf : np_faces) {
                if (f == nf) {
                  current_face = f;
                  break;
                }
            ...
            // std::cout << "exact_geodesic: checkpoint 7." << i << std::endl;

            assert(current_face != nullptr &&
                   "current face seems to be nullptr");

            auto fid = current_face->id();
            std::vector<int> initial_vertices = {F(fid, 0), F(fid, 1),
                                                 F(fid, 2)};

            // std::cout << "p: " << p.type() << ", next_p: " << next_p.type()
            //           << ", faceid: " << fid << ", i: " << path.size();

            // 面上にあるvidの一覧
            // 配列長5なら014, 124,
            // 234を結ぶ。1と4がedge、023が元のvertex。1は0と2の、3は2と4の間にある。
            // 配列長4なら013, 123を結ぶ。1がedge、023が元のvertex
            std::vector<int> vids = {};

            if (p.type() == exact_geodesic::geodesic::EDGE &&
                next_p.type() == exact_geodesic::geodesic::EDGE) {
              // 両方ともedge
              vids.resize(5);

              // std::cout << "exact_geodesic: checkpoint 8." << i << std::endl;

              // pの辺の端点
              std::vector<unsigned int> v0 = {
                  p.base_element()->adjacent_vertices()[0]->id(),
                  p.base_element()->adjacent_vertices()[1]->id()};

              // next_pの辺の端点
              std::vector<unsigned int> v1 = {
                  next_p.base_element()->adjacent_vertices()[0]->id(),
                  next_p.base_element()->adjacent_vertices()[1]->id()};

              // 両辺が共有している点、つまりvids[0]にくるべきもの
              int zero_index = 0;

              // v0, v1の中で両辺が共有していない点
              int v0_other_id = -1, v1_other_id = -1;

              // std::cout << "exact_geodesic: checkpoint 9." << i << std::endl;

              int v0size = v0.size();

              for (int k = 0; k < v0size; k++) {
                auto v = v0[k];
                for (int l = 0; l < v1.size(); l++) {
                  auto _v = v1[l];
                  if (v == _v) {
                    zero_index = v;
                    v0_other_id = v0[(k + 1) % 2];
                    v1_other_id = v1[(l + 1) % 2];
                    break;
                  }
                }
                if (k == v0size - 1) {
                  assert(v0_other_id != -1 && v1_other_id != -1 &&
                         "no zero indices");
                }
              }

              // std::cout << "exact_geodesic: checkpoint 10." << i << std::endl;

              for (int k = 0; k < 3; k++) {
                if (initial_vertices[k] == zero_index) {
                  vids[0] = initial_vertices[k];
                  vids[2] = initial_vertices[(k + 1) % 3];
                  vids[3] = initial_vertices[(k + 2) % 3];

                  if (v0_other_id == vids[2]) {
                    // pが1番の位置に来るべき
                    vids[1] = p_index;
                    vids[4] = next_p_index;
                  } else {
              ...
              exact_geodesic::geodesic::SurfacePoint edge_point;
              int edge_point_index;

              if (p.type() == exact_geodesic::geodesic::EDGE) {
                edge_point = p;
                edge_point_index = p_index;
              } else if (next_p.type() == exact_geodesic::geodesic::EDGE) {
                edge_point = next_p;
                edge_point_index = next_p_index;
              } else {
                assert(false && "the geodesic is neither edge nor vertex");
              }

              vids[1] = edge_point_index;

              std::vector<unsigned int> v0 = {
                  edge_point.base_element()->adjacent_vertices()[0]->id(),
                  edge_point.base_element()->adjacent_vertices()[1]->id()};
              ...
              for (int k = 0; k < 3; k++) {
                ...
                for (int l = 0; l < 2; l++) {
                  auto current_v = v0[l];
                  auto other_v = v0[(l + 1) % 2];

                  if (initial_vertices[k] == current_v &&
                      initial_vertices[(k + 1) % 3] == other_v) {
                    vids[0] = initial_vertices[k];
                    vids[2] = initial_vertices[(k + 1) % 3];
                    vids[3] = initial_vertices[(k + 2) % 3];
                    break_flag = true;
                ...
                assert(k < 2 && "no proper 4 vids");
              }
            } else {
              assert(false && "the point is somehow not edge nor vertex");
            }

            // std::cout << "exact_geodesic: checkpoint 11." << i << std::endl;

            fid_vid_pair.emplace(fid, vids);
          }
        }

        // 得られたvidsを元に新しいfaceを作る
        int original_face_nums = F.rows();
        int rows_count = 0;

        for (const auto& kv : fid_vid_pair) {
          auto fid = kv.first;
          auto vids = kv.second;
          ...
          if (vids.size() == 4) {
            ...
            assert(false && "no proper vid size");
          }
        }

        // std::cout << "exact_geodesic: checkpoint 12" << std::endl;

        int itrc = 0;
        F.conservativeResize(F.rows() + rows_count, 3);
        ...
        for (const auto& kv : fid_vid_pair) {
          ...
          if (vids.size() == 4) {
            // 1行増やす
            F(original_face_nums + itrc, 0) = vids[0];
            F(original_face_nums + itrc, 1) = vids[1];
            F(original_face_nums + itrc, 2) = vids[3];
            ...
            F(original_face_nums + itrc, 0) = vids[0];
            F(original_face_nums + itrc, 1) = vids[1];
            F(original_face_nums + itrc, 2) = vids[4];

            itrc++;

            F(original_face_nums + itrc, 0) = vids[1];
            F(original_face_nums + itrc, 1) = vids[3];
            F(original_face_nums + itrc, 2) = vids[4];
        ...
        // std::cout << "exact_geodesic: checkpoint 13" << std::endl;
      }

      auto indices = path_indices[j];
      Eigen::VectorXi vector(indices.size());
      for (int i = 0; i < indices.size(); i++) {
        vector(i) = indices[i];
      }
      result_indices.emplace_back(vector);

      // std::cout << "exact_geodesic: checkpoint 14" << std::endl;
    }
  }
}
}  // namespace rp
